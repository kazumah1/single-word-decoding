...
class MAEEGModule(pl.LightningModule):
    ...
    def cnn_forward(self, batch):
        ...
        x_reconstructed, x_true, masker = self.model.pretrain_forward(x, subject_ids, channel_positions)

        return x_reconstructed, x_true, masker
    
    def _run_step(self, batch, step_name):

        # SimpleConv processing
        x_reconstructed, x_true, masker = self.cnn_forward(batch)

        masker_expanded = masker.unsqueeze(1).expand_as(x_true)
        loss = self.loss(x_reconstructed[masker_expanded], x_true[masker_expanded])
        ...
        return loss, x_reconstructed, x_true
